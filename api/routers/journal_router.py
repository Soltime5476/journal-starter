...
async def update_entry(
    ...
    original_entry = await entry_service.get_entry(entry_id)
    if not original_entry:
        raise HTTPException(status_code=404, detail="Entry not found")

    updated_entry = original_entry
    for field, val in entry_update.model_dump().items():
        if val is not None:
            updated_entry[field] = val
    return await entry_service.update_entry(entry_id, updated_entry)
